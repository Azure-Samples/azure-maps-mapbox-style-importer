...
export class MapboxStyleImporter {
    ...
    private static supportedMBLayers = ['circle', 'fill', 'fill-extrusion', 'line', 'heatmap', 'raster', 'symbol'];

    /** List of supported Mapbox source types. */
    private static supportedMBSources = ['vector', 'raster', 'geojson', 'image'];
    ...
    private rasterSources: Record<string, azmaps.TileLayerOptions | azmaps.ImageLayerOptions> = {};
    ...
    public addStyle(mapboxStyle: mb.Style): void {
        ...
        if (mapboxStyle.sources) {
            Object.keys(mapboxStyle.sources).forEach(key => {
    ...
    public addSource(id: string, mapboxSource: mb.Source): void {
        ...
        switch (mapboxSource.type) {
            ...
            case 'raster':
                if (id) {
                    self.rasterSources[id] = opt as azmaps.TileLayerOptions;
                }
                break;
            case 'geojson':
    ...
    public addLayer(mapboxLayer: mb.Layer, beforeLayer?: string): azmaps.layer.Layer {
        ...
        if (MapboxStyleImporter.supportedMBLayers.indexOf(mapboxLayer.type) > -1) {
            ...
            switch (mapboxLayer.type) {
                ...
                case 'symbol':
                    //Default options.
                    var symOptions:azmaps.SymbolLayerOptions = Object.assign({}, {
                        iconOptions: {
                            image: 'none'
                        },
                        textOptions: {}
    ...
    private _addDataSource(azOptions: azmaps.DataSourceOptions, mbSource: mb.GeoJSONSourceOptions, id?: string): azmaps.source.DataSource {
        ...
        if (mbSource.data) {
            if (typeof mbSource.data === 'string') {
                ...
                //data is raw geojson.
                source.add(mbSource.data as azmaps.data.FeatureCollection);
            }
        }
    ...
    private _extractOption(key: string, mbOptions: any, azOptions: any): void {
        ...
        switch (key) {
            ...
            case 'clusterProperties':
                azOptions.clusterProperties = val as Record<string, azmaps.AggregateExpression>;
                break;
            case 'tiles':
            ...
            case 'icon-image':
                    ...
                    if(!self._map.imageSprite.hasImage(val)){
                        ...
                        if(val && loadIcon && !self._map.imageSprite.hasImage(val)){
                            self._map.imageSprite.createFromTemplate(val, val, '#1E90FF', '#fff', 1).then(()=>{});
                        }
                    }
